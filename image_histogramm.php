...
function get_Erwartung($datum, $branch_id) {
...
class roster_image_histogramm {
     ...
     *
     * @param array $Roster
     * @global int $branch_id
     * @global string $datum
     * @global array $Anwesende
     * @global float $factor_employee The number of drug packages that can be sold per employee within a certain time.
...
// We need the reader for Erwartung here or inside a seperate function file!
        $Erwartung = get_Erwartung($datum, $branch_id);
        $canvas_width = 650;
        $canvas_height = 300;
...
//        $canvas_text .= roster_image_histogramm::draw_image_dienstplan_add_headcount($outer_margin_x, $width_factor, $height_factor, $start_time, $Anwesende, $factor_employee);
        ...
        $canvas_text .= "</script>";
        print_debug_variable('$canvas_text', $canvas_text);
