...
/**
 * The Employee class represents an individual employee's information and
 * attributes.
 * It provides constructors and methods to create, access, and manage employee
 * data.
 * Employees' personal information, employment details, and branch affiliations
 * are stored within instances of this class.
 *
 * Attributes:
 ...
 * - employeeLunchBreakMinutes: Duration of the employee's lunch break in
 * minutes.
 ...
 * - employeeAbilitiesGoodsReceipt: Indicates whether the employee can handle
 * goods receipt tasks.
 * - employeeAbilitiesCompounding: Indicates whether the employee can perform
 * compounding tasks.
 * - employeeStartOfEmployment: Start date of the employee's employment.
 * - employeeEndOfEmployment: End date of the employee's employment (if
 * applicable).
 *
 * Constructors:
 * - Employee(employeeKey, employeeLastName, employeeFirstName,
 * employeeProfession, employeeWorkingHours, employeeLunchBreakMinutes,
 * employeeHolidays, employeeBranch, employeeAbilitiesGoodsReceipt,
 * employeeAbilitiesCompounding, employeeStartOfEmployment,
 * employeeEndOfEmployment):
 * Initializes attributes based on provided parameters.
 *
 * - Employee(employeeHashMap):
 * Initializes attributes based on values provided in a HashMap containing
 * employee details.
 *
 * Getter Methods:
 * - Various getter methods are provided to access individual attributes of an
 * employee object.
 *
 * Note:
 * - Date parsing is used to convert date strings to LocalDate objects for
 * employment start and end dates.
 * - Branch information is fetched from a NetworkOfBranchOffices using either
 * branch name or branch ID.
 * - Various data types are parsed from strings using appropriate conversion
 * methods.
 *
 * @author Mandelkow
 * @since 2021-11-10
...
public class Employee {
  ...
  public Employee(String employeeKey,
    ...
    try {
      this.employeeStartOfEmployment = LocalDate.parse(employeeStartOfEmployment,
          Wrapper.DATE_TIME_FORMATTER_DAY_MONTH_YEAR);
    } catch (Exception e) {
      LogCollector.debug("Employee without parsable start of employment. Keeping null value.");
    }
    try {
      this.employeeEndOfEmployment = LocalDate.parse(employeeEndOfEmployment,
          Wrapper.DATE_TIME_FORMATTER_DAY_MONTH_YEAR);

    } catch (Exception e) {
      LogCollector.debug("Employee without parsable end of employment. Keeping null value.");
    }
  }
  ...
  public Employee(HashMap<String, String> employeeHashMap) {
    ...
    if (!"".equals(employeeHashMap.get("employeeStartOfEmployment"))) {
      this.employeeStartOfEmployment = LocalDate.parse(employeeHashMap.get("employeeStartOfEmployment"),
          Wrapper.DATE_TIME_FORMATTER_YEAR_MONTH_DAY);
    }
    if (!"".equals(employeeHashMap.get("employeeEndOfEmployment"))) {
      this.employeeEndOfEmployment = LocalDate.parse(employeeHashMap.get("employeeEndOfEmployment"),
          Wrapper.DATE_TIME_FORMATTER_YEAR_MONTH_DAY);
    }
