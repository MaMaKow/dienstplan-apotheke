...
$date_sql = user_input::get_variable_from_any_input("datum", FILTER_SANITIZE_SPECIAL_CHARS, date('Y-m-d'));
...
if (filter_has_var(INPUT_POST, 'Roster')) {
    try {
        ...
        if (filter_has_var(INPUT_POST, 'submit_roster') && $session->user_has_privilege('create_roster')) {
            user_input::roster_write_user_input_to_database($RosterFromPost);
